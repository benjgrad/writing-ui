/**
 * Note Quality (NVQ) Type Definitions
 *
 * Implements the 10-point NVQ scorecard for Zettelkasten note validation.
 * Based on requirements from tags_and_metada_structure_requirements.md
 */
 ...
 * - More than 50% synthesis: 1 point
 * - Fails Wikipedia test (pure facts): 0 points
 */
...
// ============================================================================
// Extended Note Types for Quality Testing
// ============================================================================

/**
 * Extended note result with quality attributes
 */
export interface QualityExtractedNote {
...
/**
 * Ground truth expectations for quality testing
 */
export interface QualityExpectation {
  ...
  purposePattern: string | RegExp
  expectedProject: string | null // null = no specific project expected

  // Metadata expectations
  expectedStatus: NoteStatus
  expectedType: NoteType
  expectedStakeholder: Stakeholder

  // Tag expectations
  expectedFunctionalTags: string[] // e.g., ["#task/implement", "#skill/typescript"]
  forbiddenTags: string[] // Topic tags that should NOT appear

  // Connection expectations
  expectedUpwardLink: string | null // MOC or Project
  expectedSidewaysLinks: string[] // Related concepts
 ...
 */
export interface NoteQualityMetrics {
  // Overall scores
  meanNVQ: number
  medianNVQ: number
  minNVQ: number
  ...
  // Score distributions
  whyScoreDistribution: Record<number, number>
  metadataScoreDistribution: Record<number, number>
  taxonomyScoreDistribution: Record<number, number>
  connectivityScoreDistribution: Record<number, number>
  originalityScoreDistribution: Record<number, number>
...
/**
 * Quality test scenario with expectations
 */
export interface QualityTestScenario {
  name: string
  description: string
  ...
  // Quality expectations per document
...
export interface NVQEvaluationResult {
  ...
  noteContent: string
  nvqScore: NVQScore
  matchedExpectation: QualityExpectation | null
  issues: string[]
}

/**
 * Complete quality evaluation results
 */
export interface QualityEvaluationResults {
  scenarioName: string
  noteResults: NVQEvaluationResult[]
  aggregateMetrics: NoteQualityMetrics
